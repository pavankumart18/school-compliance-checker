...
const failureDetails = [
  { school: 'School 1', id: 'FAC-002', title: 'Radon Testing — Building B Overdue', evidence: 'Only 1 of 2 campus buildings tested. Building B (built 2006) has no Form DH-1777 on record. Testing overdue since 2022.', severity: 'red' },
  { school: 'School 1', id: 'IRS-002', title: 'Form 8300 — $12,000 Cash Payment', evidence: 'One family paid $12,000 cash tuition. Form 8300 was not filed within the 15-day window required by 26 U.S.C. §6050I.', severity: 'red' },
  { school: 'School 1', id: 'BSF-003', title: 'Chemical Hygiene Plan Outdated', evidence: 'CHP last updated 2021 — 4+ years stale. SDS binder in Chemistry lab missing 6 recent chemical additions. Eyewash log has a 3-week gap.', severity: 'red' },
  { school: 'School 2', id: 'ADM-001', title: 'Annual Survey — Late & Missing Notary', evidence: 'Submitted May 3 (2 days past deadline). Notary seal completely absent from the filed document. Survey is legally incomplete.', severity: 'red' },
  { school: 'School 2', id: 'PER-001', title: 'VECHS Clearance — Expired Screening', evidence: 'Staff member J. Martinez last screened June 2019 — now 5 years 8 months expired. Active student contact role. Immediate risk.', severity: 'red' },
  { school: 'School 2', id: 'HLT-001', title: 'DH-684 — Missing 7th Grade Data', evidence: 'Kindergarten immunization data complete. 7th grade section entirely blank. Submission flagged as incomplete by DOH.', severity: 'red' },
  { school: 'School 2', id: 'ADM-003', title: 'Scholarship Form — 7 Days Late', evidence: 'SCF-1 filed March 8, 2025. Statutory deadline was March 1. Seven-day gap creates potential audit exposure with Step Up.', severity: 'amber' },
  { school: 'School 2', id: 'DOL-002', title: 'FMLA Poster Not Found', evidence: 'WH-1420 poster not displayed during physical walkthrough. Required for all employers with 50+ employees under 29 CFR §825.300.', severity: 'red' },
  { school: 'School 2', id: 'BSF-001', title: 'AHERA Asbestos Plan — Overdue', evidence: 'AMP last updated 2019. Triennial reinspection overdue by 2+ years. Plan must be accessible to staff — currently in locked storage.', severity: 'red' },
  { school: 'School 2', id: 'IRS-001', title: 'Beneficial Ownership — No BOI Filing', evidence: 'No FinCEN BOI report found on record. Required under 2024 Corporate Transparency Act for all applicable entities.', severity: 'red' }
];
...
function drawComparisonChart() {
  ...
  [{label:'School 1', color:'#3b82f6'}, {label:'School 2', color:'#f59e0b'}].forEach((item, i) => {
      ...
      statuses.forEach(s => {
        ...
        svg.append('rect')
          ...
          .on('mouseenter', function(event) {
            d3.select(this).attr('opacity', 1);
            const schoolName = school === 'nb' ? 'School 1' : 'School 2';
...
function drawHeatmap() {
  ...
  const schools = ['School 1', 'School 2'];
  ...
  svg.selectAll('.hm-school')
    ...
    .text(d => d === 'School 1' ? 'School 1' : 'School 2');
  ...
  schools.forEach(school => {
    const results = school === 'School 1' ? nbResults : wmResults;
    regs.forEach(id => {
...
function drawDotPlot() {
  ...
  svg.selectAll('.nb-dot')
    ...
    .on('mouseenter', function(event, d) {
      ...
      const s = nbResults[d];
      showTooltip(`<strong>School 1</strong><br>${d}: ${regNames[d]}<br>Status: <span style="color:${statusColor[s]}">${s}</span>`, event);
    })
    .on('mousemove', (event) => showTooltip(tooltip.html(), event))
  ...
  svg.selectAll('.wm-dot')
    ...
    .on('mouseenter', function(event, d) {
      d3.select(this).transition().duration(150).attr('r', 7);
      const s = wmResults[d];
      showTooltip(`<strong>School 2</strong><br>${d}: ${regNames[d]}<br>Status: <span style="color:${statusColor[s]}">${s}</span>`, event);
    })
    .on('mousemove', (event) => showTooltip(tooltip.html(), event))
    .on('mouseleave', function() { d3.select(this).transition().duration(150).attr('r', 5); hideTooltip(); })
...
const pipelineContent = {
  ...
  2: {
    ...
    html: `
      ...
      <div class="pipe-output-stats">
        <div class="pipe-stat"><div class="pipe-stat-num">6</div><div class="pipe-stat-label">School 1 Docs</div></div>
        <div class="pipe-stat"><div class="pipe-stat-num">6</div><div class="pipe-stat-label">School 2 Docs</div></div>
        <div class="pipe-stat"><div class="pipe-stat-num">12</div><div class="pipe-stat-label">Total PDFs</div></div>
      </div>
      <table class="pipe-output-table">
        <thead><tr><th>Document</th><th>School 1</th><th>School 2</th></tr></thead>
        <tbody>
  ...
  3: {
    ...
    html: `
      ...
      <div class="pipe-terminal">
        <span class="t-blue">EXTRACTING</span> Annual_Survey_2025.pdf (School 1)...<br>
        &nbsp;&nbsp;→ filing_date: <span class="t-green">2025-04-15</span><br>
        &nbsp;&nbsp;→ notary_seal: <span class="t-green">PRESENT</span> (County A #FR-882)<br>
        &nbsp;&nbsp;→ school_code: NB-0042<br>
        &nbsp;&nbsp;→ enrollment: 847<br>
        <br>
        <span class="t-blue">EXTRACTING</span> VECHS_Clearance_Log.pdf (School 2)...<br>
        &nbsp;&nbsp;→ total_staff: 47<br>
        ...
        <br>
        <span class="t-blue">EXTRACTING</span> DH684_Immunization_Report.pdf (School 2)...<br>
  ...
  4: {
    ...
    html: `
      ...
      <div class="pipe-terminal">
        <span class="t-blue">VALIDATING</span> ADM-001 — Annual Survey (School 1)<br>
        ...
        <br>
        <span class="t-blue">VALIDATING</span> ADM-001 — Annual Survey (School 2)<br>
        &nbsp;&nbsp;Rule: Must be filed before May 1st with notary seal<br>
        ...
        <br>
        <span class="t-blue">VALIDATING</span> PER-001 — VECHS Clearance (School 2)<br>
  ...
  5: {
    ...
    html: `
      ...
      <div class="pipe-output-stats">
        <div class="pipe-stat"><div class="pipe-stat-num">87%</div><div class="pipe-stat-label">School 1</div></div>
        <div class="pipe-stat"><div class="pipe-stat-num">61%</div><div class="pipe-stat-label">School 2</div></div>
      ...
      <table class="pipe-output-table">
        <thead><tr><th>Metric</th><th>School 1</th><th>School 2</th></tr></thead>
        <tbody>
